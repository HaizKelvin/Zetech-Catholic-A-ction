  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 lg:space-y-32 pb-32">
      <motion.header 
        ...
        className="relative py-12 md:py-32 px-6 md:px-24 rounded-[40px] md:rounded-[80px] overflow-hidden bg-stone-950 text-white shadow-3xl shadow-brand-900/10 group mb-8 md:mb-12"
      >
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-10 md:gap-12">
          <div className="space-y-4 md:space-y-6">
            <h1 className="text-4xl md:text-[8rem] font-black tracking-tighter italic serif-display leading-tight md:leading-[0.8]">Sacred <span className="text-brand-600 dark:text-brand-500 not-italic uppercase font-black text-xl md:text-3xl tracking-[0.3em] block mt-2 md:mt-4">Gatherings</span></h1>
            <div className="flex items-center gap-4 md:gap-6">
              <p className="text-stone-400 font-serif italic text-base md:text-2xl opacity-80 pl-6 md:pl-10 border-l-2 border-brand-500/30">Liturgy, fellowship, and divine movement.</p>
            </div>
          </div>
          {isAdmin && (
            <div className="flex flex-wrap gap-4 md:gap-6 w-full md:w-auto">
              <motion.button 
                ...
                className="flex-1 md:flex-none justify-center bg-white/5 border border-white/10 text-stone-300 px-6 py-4 md:px-8 md:py-5 rounded-[22px] md:rounded-[28px] font-black uppercase tracking-[0.3em] text-[9px] md:text-[10px] backdrop-blur-xl hover:bg-white/10 transition-all"
              >
                Clear Past
              </motion.button>
              <motion.button 
                ...
                className="flex-1 md:flex-none justify-center bg-brand-900 text-white px-6 py-4 md:px-8 md:py-5 rounded-[22px] md:rounded-[28px] font-black uppercase tracking-[0.3em] text-[9px] md:text-[10px] shadow-3xl shadow-brand-900/40 hover:bg-brand-800 transition-all"
              >
                Post Event
              </motion.button>
            </div>
      ...
      <div className="grid grid-cols-1 gap-8 md:gap-12 lg:gap-20 px-4 md:px-0">
        <AnimatePresence>
          {events.length === 0 ? (
            <div className="text-center py-16 md:py-24 glass rounded-[32px] md:rounded-[40px]">
               <Calendar className="w-12 h-12 md:w-16 md:h-16 text-stone-100 dark:text-stone-800 mx-auto mb-4" />
               <p className="text-stone-400 font-medium tracking-wide italic text-sm md:text-base">No scheduled events at the moment.</p>
            </div>
          ) : (
            events.map((ev) => (
              <motion.div
              ...
              >
                ...
                <div className="flex-1 space-y-4 md:space-y-6">
                  <div className="flex justify-between items-start">
                    <h3 className="text-2xl md:text-3xl font-bold text-stone-900 dark:text-stone-100">{ev.title}</h3>
                    {isAdmin && (
                  ...
                  <p className="text-stone-600 dark:text-stone-400 leading-relaxed max-w-2xl text-sm md:text-base">{ev.description}</p>
                  <button className="flex items-center gap-2 text-brand-600 font-bold text-[10px] md:text-sm uppercase tracking-widest group/btn border-b-2 border-brand-100 dark:border-brand-900/30 pb-1 hover:border-brand-600 transition-all w-fit">
                    Set Reminder
